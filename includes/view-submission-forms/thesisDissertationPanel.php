...
$publication_date = $fileInfo['publication_date'];
...
<div class="row">
    ...
    <div class="col-lg-10 px-5 col-md-12 col-xs-12 main-column" id="thesisDissertationPanel">
        ...
        <form name="thesis-form" action="<?php echo "../../src/process/user-update-submission.php?id=".$fileInfo['file_id']."&coauthor_id=".$fileInfo['coauthor_group_id']; ?>" method="POST" enctype="multipart/form-data">
            ...
            <div class="col">
                <?php if (!empty($fileInfo['file_dir']) && !@file_exists(rtrim($_SERVER['DOCUMENT_ROOT'], '/\\') . base_url() . '/src/' . $fileInfo['file_dir'])): ?><span class="text-danger fw-semibold my-2 d-inline-block"><i class="fas fa-triangle-exclamation me-1"></i>Research Paper (<?php echo htmlspecialchars($fileInfo['file_name']) ?>) — file missing on server</span><?php else: ?><p class="my-3"><a href="../../src/<?php echo htmlspecialchars($fileInfo['file_dir']); ?>" target="_blank"><span class="attachment-label">Research Paper</span><?php echo htmlspecialchars($fileInfo['file_name']); ?></a></p><?php endif; ?>
                <input class="form-control my-2" type="file" name="fileSubmit" accept=".pdf, .docx">
                <?php if (!empty($fileInfo['file_dir2']) && !@file_exists(rtrim($_SERVER['DOCUMENT_ROOT'], '/\\') . base_url() . '/src/' . $fileInfo['file_dir2'])): ?><span class="text-danger fw-semibold my-2 d-inline-block"><i class="fas fa-triangle-exclamation me-1"></i>Questionnaire (<?php echo htmlspecialchars($fileInfo['file_name2']) ?>) — file missing on server</span><?php else: ?><p class="my-3"><a href="../../src/<?php echo htmlspecialchars($fileInfo['file_dir2']); ?>" target="_blank"><span class="attachment-label">Questionnaire</span><?php echo htmlspecialchars($fileInfo['file_name2']); ?></a></p><?php endif; ?>
