...
if (empty($_POST['textFieldEmail'] && $_POST['textFieldPassword'])) {
    ...
    if ($statement = $connection->prepare('SELECT user_id, first_name, last_name, department, password, user_type, is_suspended FROM users WHERE email = ?')) {
        $statement->bind_param('s', $_POST['textFieldEmail']);
        $statement->execute();
        $statement->store_result();

        if ($statement->num_rows > 0) {
            $statement->bind_result($userid, $firstName, $lastName, $department, $password, $user_type, $is_suspended);
            $statement->fetch();

            if ($is_suspended) {
                $arr = array('response' => "account_suspended");
                header('Content-Type: application/json');
                echo json_encode($arr);
                exit();
            }
